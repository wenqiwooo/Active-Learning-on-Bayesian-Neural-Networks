...
from stochastic import (
  get_random_normal_variable,
  gaussian_mixture_nll,
  ExternallyParameterisedLSTM,
)
import logging
...
class BayesianRNN(object):
  """
  An implementation of an RNN trained using Variational Bayes for RNNs, 
  introduced in: Bayesian Recurrent Neural Networks, 
  ...
  def build_rnn(self):
    ...
    self.targets = tf.placeholder(
        tf.int32, [self.batch_size, self.num_steps])
    self.initial_lstm_memory = tf.placeholder(
        tf.float32, [self.batch_size, self.hidden_size])
    self.initial_lstm_state = tf.placeholder(
        tf.float32, [self.batch_size, self.hidden_size])
    self.initial_state = LSTMStateTuple(
        self.initial_lstm_memory, self.initial_lstm_state)
    ...
    inputs = [tf.squeeze(single_input, [1]) \
    ...
    w_shape = [self.embedding_size + self.hidden_size, 4 * self.hidden_size]
    b_shape = [4 * self.hidden_size]
    
    with tf.variable_scope('phi_rnn'):
      phi_w, phi_w_mean, phi_w_std = get_random_normal_variable(
          'phi_w', 0., self.init_scale, w_shape, dtype=tf.float32)
      phi_b, phi_b_mean, phi_b_std = get_random_normal_variable(
          'phi_b', 0., self.init_scale, b_shape, dtype=tf.float32)
      tf.summary.image(
          'phi_mean', tf.reshape(phi_w_mean, [1] + w_shape + [1]),
          max_outputs=1, collections=['IMAGE'])
      tf.summary.image(
          'phi_std', tf.reshape(phi_w_std, [1] + w_shape + [1]),
          max_outputs=1, collections=['IMAGE'])
      phi_cell = ExternallyParameterisedLSTM(
          phi_w, phi_b, num_units=self.hidden_size)

    with tf.variable_scope('softmax_weights'):
      softmax_w, softmax_w_mean, softmax_w_std = get_random_normal_variable(
          'softmax_w', 0., self.init_scale,
          [self.hidden_size, self.vocab_size],
          dtype=tf.float32)
      softmax_b, softmax_b_mean, softmax_b_std = get_random_normal_variable(
          'softmax_b', 0., self.init_scale,
          [self.vocab_size],
          dtype=tf.float32)

    # Sample from posterior and assign to LSTM weights
    logger.info('Resampling weights using Posterior Sharpening')
    posterior_weights = self.sharpen_posterior(
        inputs, phi_cell, [phi_w, phi_b], [softmax_w, softmax_b])
    ...
    tf.summary.image(
        'sharpening_difference',
        tf.reshape(theta_w_mean - phi_w_mean, [1] + w_shape + [1]),
        max_outputs=1,
        collections=['IMAGE'])

    logger.info('Building LSTM cell with new weights sampled from posterior')
    with tf.variable_scope('theta_lstm'):
      theta_cell = ExternallyParameterisedLSTM(
          theta_w, theta_b, num_units=self.hidden_size)

    outputs, final_state = static_rnn(
        theta_cell, inputs, initial_state=self.initial_state)

    self.final_lstm_memory = final_state.c
    self.final_lstm_state = final_state.h

    negative_log_likelihood = self.get_negative_log_likelihood(
        outputs, posterior_softmax_w, posterior_softmax_b)
    ...
    for theta, phi in zip(
        [
          theta_w_mean,
          theta_b_mean,
          posterior_softmax_w_mean,
          posterior_softmax_b_mean
        ],
        [
          phi_w_mean,
          phi_b_mean,
    ...
    phi_kl = 0.
    for weight, mean, std in [
          [phi_w, phi_w_mean, phi_w_std],
      ...
      phi_kl += self.compute_kl_divergence(
        (mean, std), (tf.zeros_like(mean), tf.ones_like(std) * 0.01))

    tf.summary.scalar('phi_kl', phi_kl)

    self.cost = negative_log_likelihood + \
        (theta_kl / self.batch_size) + \
        (phi_kl / self.batch_size*self.num_steps)
    self.inference_cost = self.mean_field_inference(
        inputs, phi_w_mean, phi_b_mean, softmax_w_mean, softmax_b_mean)
    tf.summary.scalar(
        'sharpened_word_perplexity',
        tf.minimum(1000.0, tf.exp(self.cost/self.num_steps)))
    tf.summary.scalar(
        'unsharpened_val_perplexity',
        tf.exp(self.inference_cost/self.num_steps), 'VAL')
  ...
  def sharpen_posterior(self, inputs, cell, cell_weights, softmax_weights):
    ...
    gradients, _ = tf.clip_by_global_norm(
        tf.gradients(cost, all_weights), self.max_grad_norm)
    new_weights = []
    new_parameters = []
    parameter_name_scopes = [
      'phi_w_sample',
      'phi_b_sample',
      'softmax_w_sample',
      'softmax_b_sample',
    ]
    for (cell_weight, log_likelihood_grad, scope) in zip(
        all_weights, gradients, parameter_name_scopes):

      with tf.variable_scope(scope):
        # We want each parameter to use different smoothing weights.
        new_hierarchical_posterior, new_posterior_mean = self.resample(
            cell_weight, log_likelihood_grad)
    ...
    return theta_weights, posterior_softmax_weights, theta_parameters, \
        softmax_parameters
  ...
  def resample(weight, gradient):
    ...
    smoothing_variable = tf.get_variable(
        'posterior_mean_smoothing',
    ...
    new_posterior_std = 0.02 * tf.random_normal(
        weight.get_shape(), mean=0.0, stddev=1.0)
  ...
  def mean_field_inference(self, inputs, mean_w, mean_b, softmax_w, softmax_b):
    ...
    :return:
    """
    cell = ExternallyParameterisedLSTM(
        mean_w, mean_b, num_units=self.hidden_size)
    outputs, final_state = static_rnn(
        cell, inputs=inputs, initial_state=self.initial_state)
  ...
  def run_train_step(self, sess, inputs, targets, state, memory, step):
    if step % self.summary_frequency == 0:
      summary, cost, train_step, state, memory, _ = sess.run(
          [
            self.summary,
            self.cost,
            self.global_step,
            self.final_lstm_state,
            self.final_lstm_memory,
            self.train_op
          ],
          {
            self.input_data: inputs,
            self.targets: targets,
            self.initial_lstm_state: state, 
            self.initial_lstm_memory: memory
          })
    else:
      cost, train_step, state, memory, _ = sess.run(
          [
            self.cost,
            self.global_step,
            self.final_lstm_state,
  ...
  def run_eval_step(self, sess, inputs, targets, state, memory, step):
    if step % self.summary_frequency == 0:
      summary, cost, val_step, state, memory = sess.run(
          [
            self.summary,
            self.inference_cost,
            self.global_step,
            self.final_lstm_state_val,
            self.final_lstm_memory_val
          ],
          {
            self.input_data: inputs,
            self.targets: targets,
      ...
      cost, val_step, state, memory = sess.run(
          [
            self.inference_cost,
            self.global_step,
            self.final_lstm_state_val,
  ...
  def run_image_summary(self, sess, inputs, targets, state, memory):
    return sess.run(
      [self.image_summary, self.global_step],
      {
        self.input_data: inputs,
        self.targets: targets,
        self.initial_lstm_state: state,
